...
if ($_GET['date']) {
    ...
    // date only
    $getSql = "SELECT sales.customer_id,sales_detail.product_id,SUM(sales_detail.quantity) AS qty FROM sales_detail LEFT JOIN sales ON sales.id = sales_detail.sales_id WHERE customer_id=:cid AND sales_detail.created_at>'$startDate' AND sales_detail.created_at<'$stopDate' GROUP BY product_id";
    $getPdo = $pdo->prepare($getSql);
    // $getPdo->execute();
...
<div class="row">
    ...
    <div class="col-md-3">
        <select class="form-control" name="" id="customer" onclick="filterCustomer()">
            <option value="0">All</option>
            <?php foreach ($customers as $customer) : ?>
                <option value="<?= $customer->id ?>" <?php if ($customer->id == $customerId){echo "selected";} ?>><?= $customer->name ?></option>
            <?php endforeach; ?>
        </select>
    </div>
</div>
